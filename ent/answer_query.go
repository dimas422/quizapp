...
func (_q *AnswerQuery) QueryAttemptAnswers() *AttemptAnswerQuery {
...
func (_q *AnswerQuery) WithAttemptAnswers(opts ...func(*AttemptAnswerQuery)) *AnswerQuery {
	query := (&AttemptAnswerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AnswerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Answer, error) {
	...
	if query := _q.withAttemptAnswers; query != nil {
		if err := _q.loadAttemptAnswers(ctx, query, nodes,
			func(n *Answer) { n.Edges.AttemptAnswers = []*AttemptAnswer{} },
			func(n *Answer, e *AttemptAnswer) { n.Edges.AttemptAnswers = append(n.Edges.AttemptAnswers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AnswerQuery) loadAttemptAnswers(ctx context.Context, query *AttemptAnswerQuery, nodes []*Answer, init func(*Answer), assign func(*Answer, *AttemptAnswer)) error {
	...
	query.Where(predicate.AttemptAnswer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(answer.AttemptAnswersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.answer_attempt_answers
		if fk == nil {
			return fmt.Errorf(`foreign-key "answer_attempt_answers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "answer_attempt_answers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
